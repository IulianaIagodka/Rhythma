...
| Native iOS project | **Not committed** — Expo managed / EAS prebuild. There is **no** `ios/Info.plist` in git; permissions and bundle id come from `app.json` / `app.config.js` and are generated at build time. |

Source license: root `LICENSE` (proprietary, All Rights Reserved). App Store end-user terms are Apple’s Standard EULA (or custom ASC EULA), not this file.
...
LICENSE                 # Source-code license (proprietary)
...
assets/
# no ios/ or android/ trees — generated by EAS prebuild
```
